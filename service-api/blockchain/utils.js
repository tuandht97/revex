...
const OrganizationClient = class extends EventEmitter {
  ...
  constructor(channelName, ordererConfig, peerConfig, caConfig, admin) {
    ...
    const realestatePeer = this._client.newPeer(config.realestateOrg.peer.url, {
      pem: config.realestateOrg.peer.pem,
      'ssl-target-name-override': config.realestateOrg.peer.hostname
    });

    const regulatorPeer = this._client.newPeer(config.regulatorOrg.peer.url, {
      pem: config.regulatorOrg.peer.pem,
      'ssl-target-name-override': config.regulatorOrg.peer.hostname
    });
  ...
  async invoke(chaincodeId, fcn, ...args) {
    ...
    try {
      const request = {
        ...
        args: marshalArgs(args),
        txId: txId,
        requiredOrgs: ['RealEstateOrgMSP', 'RegulatorOrgMSP', 'TraderOrgMSP']
      };
