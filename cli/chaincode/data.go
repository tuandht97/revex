...
type sellTritAdvertisingContract struct {
	...
	Time   string 	 `json:"time"`    // thời điểm mua
...
func (contract sellTritAdvertisingContract) changeAmount(stub shim.ChaincodeStubInterface, amount int, seller *shareholder) error {
	...
	currentValue, err := getHT(stub, []string{prefixSellTritAdvertisingContract + contract.UUID, strconv.Itoa(contract.Amount)})
	if err != nil {
		return err
	}
	currentValue -= amount
	if currentValue == 0 {
		...
		err = deleteHT(stub, []string{prefixSellTritAdvertisingContract + contract.UUID})
		if err != nil {
			return err
		}
		...
		args := []string{prefixSellTritAdvertisingContract + contract.UUID, strconv.Itoa(amount),"-"}

		err = updateHT(stub, args)
		if err != nil {
			return err
		}
